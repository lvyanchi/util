...
public class AutoDeploy {
	...
	public static void main(String[] args) {
		rewriteIndex(BaseConst.SCRIPT_PATTERN_BUILD, "build");
		rewriteIndex(BaseConst.SCRIPT_PATTERN_VENDOR, "vendors");
		renameBuildFile();
		rewritePackageJson();
	}
	...
	public static void rewriteIndex(String pattern, String str) {
		String fileText = FileUtil.readTxt(BaseConst.LBYC_INDEX_PATH);
		...
		while(matcher.find()){
			...
			try {
				...
				FileUtil.writeToFile(newFileText, BaseConst.LBYC_INDEX_PATH);
	...
	public static void renameBuildFile(){
		...
		if(!buildName.equals("build.js")){
			File file = new File(BaseConst.BUILD_JS_PATH);
			file.renameTo(new File(BaseConst.BUILD_JS_PATH + new Random().nextInt(10000)));
			File randomBuildFile = new File(BaseConst.LBYC_STATIC_PATH + "js/" + buildName);
			randomBuildFile.renameTo(new File(BaseConst.BUILD_JS_PATH));
		}
	}
	...
	public static void rewritePackageJson() {
		try {
			String fileText = FileUtil.readTxt(BaseConst.PACKAGE_JSON_PATH);
			...
			FileUtil.writeToFile(newFileText, BaseConst.PACKAGE_JSON_PATH);
		} catch (UnknownHostException e) {
			e.printStackTrace();
		}
	}
